...
@export var staminaRegenDelay := 1.0
...
@export var gunAmmo := {
	...
	1: {"magazine": 17, "ammo": 51}
}
@export var heal_cooldown_time: float = 8.0  # seconds between heals

var heal_on_cooldown: bool = false
...
func _refill_all_ammo_server():
	...
	_sync_all_ammo_to_client.rpc_id(get_multiplayer_authority(), gunAmmo)
...
func handle_heal(delta):
	...
	if Input.is_action_just_pressed("heal") and heals > 0:
		...
		if weapon:
			weapon.visible = false
			weapon.set_process(false)
		$PlayerHead/PlayerCamera/heal1.visible = true
		...
		$PlayerHead/PlayerCamera/heal1.visible = false
		if weapon:
			weapon.visible = true
...
func get_pistol() -> Node3D:
	return $WeaponContainer/Revolver
...
func handle_melee(delta):
	...
	if Input.is_action_just_pressed("melee"):
		...
		$PlayerHead/PlayerCamera/knife1.visible = true
		var anim := $PlayerHead/PlayerCamera/knife1/AnimationPlayer
		anim.play("slice")
		...
		melee_hit_bodies.clear()
		$PlayerHead/PlayerCamera/knife1.visible = false
		if current_weapon_node:
...
func _on_melee_body_entered(body):
	...
	if body.has_method("take_damage") and not body.is_in_group("players"):
		if multiplayer.is_server():
			...
			request_melee_damage.rpc_id(1, body.get_path(), 100, self.get_path())
